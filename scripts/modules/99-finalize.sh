...
show_setup_summary() {
    ...
    echo "     After logging back in, check:"
    echo "     • Virtual Desktops: System Settings → Workspace Behavior → Virtual Desktops"
    echo "       (Should show 4 workspaces if configured)"
    ...
    echo ""
    echo "     ${YELLOW}Note:${NC} If settings don't apply, you may need to:"
    echo "     • Delete ~/.config/plasma* files and re-run setup from TTY (Ctrl+Alt+F2)"
    echo "     • Or manually reconfigure affected settings"
    echo ""
    ...
    echo "  ${GREEN}5.${NC} Set display resolution (if needed)"
    echo "     System Settings → Display & Monitor → Display Configuration"
    echo "     Set to your preferred resolution (e.g., 1920x1080)"
    echo ""

    echo "  ${GREEN}6.${NC} Test Docker (if installed)"
    echo "     • Add user to docker group: sudo usermod -aG docker $USER"
    echo "     • Log out and log back in"
    echo "     • Test: docker run hello-world"
    echo ""

    echo "  ${GREEN}7.${NC} Test Claude Code (if installed)"
    echo "     • Run: claude --version"
    echo "     • First time setup: claude (will prompt for API key)"
    echo "     • List available agents: ls ~/.claude/agents/"
    echo ""

    echo "  ${GREEN}8.${NC} Test other applications"
